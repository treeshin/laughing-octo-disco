...
for filename in filenames:
    ...
    bkg[i,3] = sum(ew1_high)/len(ew1_low)
    bkg[i,4] = sum(ew2_low)/len(ew2_low)
    bkg[i,5] = sum(ew2_high)/len(ew2_high)
    plt.axhline(y=bkg[i,2], color='r', linestyle='-', alpha=0.3)
    i+=1
...
for filename in filenames:
    ...
    k+=1
    plt.plot(time, ew1_low, '.', color='black', lw=5, alpha=0.3, label=filename)
...
plt.title('Co-60 (EW_Low, P1, 5km/h)', fontdict=title_font, loc='center', pad=15)
